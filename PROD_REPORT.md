...
- 测试时间：2026-09-09 07:26:24
- 服务地址：`http://127.0.0.1:10011`（容器 tg-webdav，5 bot 池 + 自建 TG 代理）
...
| 指标 | 数值 |
|---|---|
| 上传 60MB | 7.82s (7.67 MB/s) |
| 全量下载 | 7.24s (8.29 MB/s) |
| 起播吞吐 | 23.76 MB/s |
| seek 吞吐 | 22.00 MB/s |
| 平均吞吐 | 16.03 MB/s |
...
真实环境里**一次 `getFile` 要 3 秒**左右（自建代理 RTT）。优化前每个分片串行各等一次
getFile，3 片文件的首次下载光等 getFile 就耗掉 ~9s。优化后在**首片下载的同时**后台并发
预取其余分片的 `file_path`，把这段 RTT 完全藏进首片下载时间里。

| 指标 | 优化前 | 优化后 | 提升 |
|---|---|---|---|
| 全量下载 60MB(3片，缓存冷) | 17.02s | **7.24s** | **快 2.35×** |
| 全量吞吐 | 3.52 MB/s | **8.29 MB/s** | **+135%** |
| 分片1 耗时 | ~6.0s(含 getFile 3s) | **0.836s** | -86% |
| 分片2 耗时 | ~5.0s(含 getFile 4s) | **0.814s** | -84% |
| 分片间隔 | 0.000s | 0.000s | 持平(均已无缝) |

日志佐证（预取与主下载并行，`getFile` 并发而非串行）：
```
...
> 注：`TTFB` 首片仍约 4.5s，因为首片的 getFile(3s) + Range 首字节(1.5s) 在关键路径上，
> 预取无法消除首片自身的 getFile。要进一步降低首播等待，需从代理侧优化 getFile 响应，
> 或让 `file_path` 缓存持久化（重启后仍有效）。

## 播放平滑度（真实带宽）

| 场景 | TTFB | 最大等待 | p95 | 卡顿>300ms | 严重>1s | 吞吐 |
|---|---|---|---|---|---|---|
| 起播 bytes=0- | 343ms | 346ms | 3ms | 2次 | 0次 | 23.76MB/s |
| seek 60% | 354ms | 355ms | 2ms | 1次 | 0次 | 22.00MB/s |
| 全量(无Range) | 4553ms | 338ms | 7ms | 2次 | 0次 | 8.29MB/s |
...
| put.big(201/204) | PASS | status=204 |
